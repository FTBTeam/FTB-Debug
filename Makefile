...
build/darwin/amd64:
	...
	wails build --platform darwin/amd64 -o ftb-debug-darwin-arm64 -trimpath -ldflags "-s -w -X 'main.GitCommit=$(GITHUB_SHA_SHORT)' -X 'main.Version=$(GITHUB_REF_NAME)'"

build/darwin/arm64:
	$(info Building debug tool for darwin arm64)
	wails build --platform darwin/arm64 -o ftb-debug-darwin-amd64 -trimpath -ldflags "-s -w -X 'main.GitCommit=$(GITHUB_SHA_SHORT)' -X 'main.Version=$(GITHUB_REF_NAME)'"
